...
namespace PaaspopService.Application.Performances.Commands.UpdatePerformance
{
    ...
    {
        private IPerformancesRepository _performancesRepository;

        public UpdatePerformanceHandler(IMapper mapper, IPerformancesRepository performancesRepository, IMediator mediator = null) : base(mapper, mediator)
        {
            _performancesRepository = performancesRepository;
        }

        public override async Task<Performance> Handle(UpdatePerformanceCommand request, CancellationToken cancellationToken)
        {
