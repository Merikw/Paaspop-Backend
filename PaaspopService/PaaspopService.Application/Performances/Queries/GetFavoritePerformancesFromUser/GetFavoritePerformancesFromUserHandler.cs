...
{
    public class
        GetFavoritePerformancesFromUserHandler : GeneralRequestHandler<GetFavoritePerformancesFromUserQuery,
            List<Performance>>
    {
        private readonly IUsersRepository _usersRepository;

        public GetFavoritePerformancesFromUserHandler(IMapper mapper, IUsersRepository usersRepository,
            IMediator mediator) : base(mapper, mediator)
        {
            _usersRepository = usersRepository;
        }

        public override async Task<List<Performance>> Handle(GetFavoritePerformancesFromUserQuery request,
            CancellationToken cancellationToken)
        {
            ...
            {
                var performance =
                    await Mediator.Send(new GetPerformanceByIdQuery {Id = performanceId}, cancellationToken);
